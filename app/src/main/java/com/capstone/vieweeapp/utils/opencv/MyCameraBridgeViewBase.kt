 ...
 */
abstract class MyCameraBridgeViewBase : SurfaceView, SurfaceHolder.Callback {

    companion object {
        private const val TAG = "CameraBridge"
        protected const val MAX_UNSPECIFIED = -1
        private const val STOPPED = 0
        private const val STARTED = 1
        const val CAMERA_ID_ANY = -1
        const val CAMERA_ID_BACK = 99
        const val CAMERA_ID_FRONT = 98
        const val RGBA = 1
        const val GRAY = 2
    }
    ...
    private var mListener: MyCvCameraViewListener2? = null
    private var mSurfaceExist = false
    private val mSyncObject = Any()
    protected var mFrameWidth = 0
    protected var mFrameHeight = 0
    protected var mMaxHeight: Int
    protected var mMaxWidth: Int
    protected var mScale = 0f
    protected var mPreviewFormat: Int = RGBA
    protected var mCameraIndex: Int = CAMERA_ID_ANY
    protected var mEnabled = false
    protected var mCameraPermissionGranted = false
    ...
    constructor(context: Context?, attrs: AttributeSet) : super(context, attrs) {
        ...
        if (styledAttrs.getBoolean(
                R.styleable.CameraBridgeViewBase_show_fps,
                false
            )
        ) enableFpsMeter()
    ...
    interface MyCvCameraViewListener {
        /**
    ...
    interface MyCvCameraViewListener2 {
        /**
         * This method is invoked when camera preview has started. After this method is invoked
         * the frames will start to be delivered to client via the onCameraFrame() callback.
         * @param width -  the width of the frames that will be delivered
         ...
         * TODO: pass the parameters specifying the format of the frame (BPP, YUV or RGB and etc)
         */
        fun onCameraFrame(inputFrame: MyCvCameraViewFrame): Mat?
    }

    protected inner class MyCvCameraViewListenerAdapter(private val mOldStyleListener: MyCvCameraViewListener) :
        MyCvCameraViewListener2 {
        ...
        override fun onCameraFrame(inputFrame: MyCvCameraViewFrame): Mat? {
            var result: Mat? = null
            when (mPreviewFormat) {
                RGBA -> result =
                    mOldStyleListener.onCameraFrame(inputFrame.rgba())

                GRAY -> result =
                    mOldStyleListener.onCameraFrame(inputFrame.gray())
    ...
    override fun surfaceChanged(arg0: SurfaceHolder, arg1: Int, arg2: Int, arg3: Int) {
        ...
        synchronized(mSyncObject) {
            if (!mSurfaceExist) {
                ...
                mSurfaceExist = false
                checkCurrentState()
                /* Now use new surface. Say we have it now */mSurfaceExist = true
                checkCurrentState()
            }
        }
    }
     ...
     * @param listener
     */
    fun setCvCameraViewListener(listener: MyCvCameraViewListener2?) {
        mListener = listener
    }

    fun setCvCameraViewListener(listener: MyCvCameraViewListener) {
        val adapter = MyCvCameraViewListenerAdapter(listener)
    ...
    fun SetCaptureFormat(format: Int) {
        mPreviewFormat = format
        if (mListener is MyCvCameraViewListenerAdapter) {
            val adapter = mListener as MyCvCameraViewListenerAdapter
            adapter.setFrameFormat(mPreviewFormat)
        }
    }
    ...
    private fun checkCurrentState() {
        ...
        val targetState: Int
        targetState =
            if (mEnabled && mCameraPermissionGranted && mSurfaceExist && visibility == VISIBLE) {
                STARTED
            } else {
                STOPPED
            }
    ...
    private fun onEnterStartedState() {
        ...
        /* Connect camera */if (!connectCamera(width, height)) {
     ...
     */
    protected fun deliverAndDrawFrame(frame: MyCvCameraViewFrame) {
        val modified: Mat?
        modified = if (mListener != null) {
        ...
        if (modified != null) {
            try {
                ...
                Log.e(
                    TAG,
                    "Bitmap type: " + mCacheBitmap!!.width + "*" + mCacheBitmap!!.height
                )
                Log.e(
                    TAG,
                    "Utils.matToBitmap() throws an exception: " + e.message
                )
                bmpValid = false
            }
        }
     ...
     */
    protected fun calculateCameraFrameSize(
        supportedSizes: List<*>,
        ...
        var calcHeight = 0
        val maxAllowedWidth =
            if (mMaxWidth != MAX_UNSPECIFIED && mMaxWidth < surfaceWidth) mMaxWidth else surfaceWidth
        val maxAllowedHeight =
            if (mMaxHeight != MAX_UNSPECIFIED && mMaxHeight < surfaceHeight) mMaxHeight else surfaceHeight
